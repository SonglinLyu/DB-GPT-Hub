...
pred_log="dbgpt_hub_sql/output/logs/pred_test_${current_date}.log"
start_time=$(date +%s)
...
CUDA_VISIBLE_DEVICES=0,1  python dbgpt_hub_sql/predict/predict.py \
    ...
    --predicted_input_filename dbgpt_hub_sql/data/example_text2sql_dev.json \
    --checkpoint_dir dbgpt_hub_sql/output/adapter/CodeLlama-7b-sql-lora \
    --predicted_out_filename dbgpt_hub_sql/output/pred/pred_codellama7b.sql >> ${pred_log}
